...
load("potential_cams_mesh.Rdata")

class(potential_cams_mesh_SigmaM)
...
#fn to create the buffered mask from df of coordinates with 2/3 sigma spacing
...
#must pass mask for specific area

#MAY NEED AN ARGUMENT THAT SPECIFIES POLY OR NOT
...
#test
test <- Sims1(mask = potential_cams_mesh_SigmaF, alltraps = potential_cams_mesh_SigmaF, nT = 50, sigmaF = sigmaF, sigmaM = sigmaM, 
                lambda0F = lambda0F, lambda0M = lambda0M, DF = DF, DM = DM, nreps = 1, raw = TRUE, ngen = 3 )
