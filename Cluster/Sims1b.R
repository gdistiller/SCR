...
grid.design <- function(mask, lambda0, sigma, nT){
  ...
  #df to store results
  #needed in big sim that stores everything
...
#using M spacing for potential traps, raw = TRUE to keep data
sim.results1.M <- Sims1(alltraps = alltrpsM, nT = nT, sigmaF = sigmaF, sigmaM = sigmaM, 
                        lambda0F = lambda0F, lambda0M = lambda0M, DF = DF, DM = DM, raw = TRUE, nreps = 100, ngen = 100)

save.image("Sims1b.RData")
